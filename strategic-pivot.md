...
**Skill cleanup COMPLETED (2026-06-04):** `deliverable-review` decoupled — stripped the `_recall resolve`/`memory-index` recall block, the "Memory index pattern verification" cross-cutting check, the `lifecycle.md`/`session.md`/`/design`-routing "Next steps", and the `continuation:` frontmatter + Continuation tail-call section. Its ISO-25010/IEEE-1012 review core (inventory→gap→axes→severity report) is framework-agnostic and kept; next-steps is now report-only ("user reads severity counts, user decides"). `prioritize` **RETIRED (deleted), not decoupled** — user decision 2026-06-04: its entire input (`session.md` task queue + `edify _worktree ls` plan statuses) was dead, leaving nothing to score; WSJF scoring math survives in `plans/prototypes/score.py` (salvage dir). CLAUDE.md skill list corrected to live set (`proof, ground, requirements, deliverable-review, token-efficient-bash`) — it had also still listed the already-deleted `handoff`/`brief`. Live plugin skills now: `deliverable-review, ground, proof, requirements, token-efficient-bash`. Precommit green. Surviving-skills list below is now stale (lists `handoff`/`brief`/`prioritize` as survivors — all since deleted).
